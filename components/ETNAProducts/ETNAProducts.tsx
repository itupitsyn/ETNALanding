...
export const ETNAProducts: FC = () => {
  const commonCardClass =
    'xs:aspect-auto xs:h-[391px] xs:grow relative flex aspect-square flex-col items-start justify-end px-6 py-5.5';

  return (
    <div className="xs:flex-row container flex flex-col gap-2.5 py-8">
      <div
        className={cn(
          'bg-[url(/movemate-bg.png)] bg-size-[1500px_auto] bg-position-[53%_40%] bg-no-repeat',
          commonCardClass,
        )}
      >
        <MovemateLogo className="absolute top-1/2 left-1/2 -translate-1/2" />
      ...
      <div
        className={cn('bg-black bg-[url(/secure-bg.png)] bg-size-[700px_auto] bg-top bg-no-repeat', commonCardClass)}
      >
      ...
      <div
        className={cn(
          'bg-[radial-gradient(77.09%_179.29%_at_-19.41%_94.55%,rgba(135,168,209,0.3)_0%,rgba(0,0,0,0)_100%),linear-gradient(0deg,#00081D,#00081D)]',
          commonCardClass,
        )}
      >
        <Image
        ...
        <p className="text-lighter z-1 text-[22px] leading-6 tracking-tight whitespace-pre">
          {'Единая платформа\nдля работы с ИИ'}
        </p>
      </div>
    </div>
